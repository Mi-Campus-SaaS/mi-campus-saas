...
const AnnouncementsPage: React.FC = () => {
  ...
  const updateMut = useMutation({
    mutationFn: ({
      ...
      id: string;
      input: { content?: string; publishAt?: string };
      ...
      {user && (user.role === "admin" || user.role === "teacher") && (
            ...
            if (!result.success) {
              ...
              for (const issue of result.error.issues) {
                if (issue.path[0] === "content")
                  fieldErrors.content = t(issue.message);
                if (issue.path[0] === "publishAt")
                  fieldErrors.publishAt = t(issue.message);
              }
        ...
        >
          <div className="flex-1 min-w-[240px]">
            ...
            />
            {errors.content && (
              <div className="text-xs text-red-600">{errors.content}</div>
            )}
          </div>
          <div>
            ...
            />
            {errors.publishAt && (
              <div className="text-xs text-red-600">{errors.publishAt}</div>
            )}
          </div>
          <button
            ...
            {user && (user.role === "admin" || user.role === "teacher") && (
              <div className="flex gap-2">
                <button
                  ...
                  onClick={() => {
                    const content =
                      prompt(t("prompt_edit_content"), a.content) || "";
                    if (!content.trim()) return;
                    const publishAt =
                      prompt(
                        t("prompt_edit_publish_at"),
                        new Date(a.publishAt).toISOString()
                      ) || "";
                    updateMut.mutate({
                      id: a.id,
                      input: {
